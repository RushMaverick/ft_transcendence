...
class FriendsViewSet(viewsets.ModelViewSet):
    ...
    def send_request(self, request):
        ...
        if FriendRequest.objects.filter(from_user=user, to_user=friend_user, accepted=False).exists() or FriendRequest.objects.filter(from_user=friend_user, to_user=user,  accepted=False).exists():
            return Response({"Warning": "This Friend Request has already been sent or received."}, status=status.HTTP_409_CONFLICT)
            ...
            return Response({"Detail": "They are friends already"}, status=status.HTTP_409_CONFLICT)
